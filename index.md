## essurvey ![](reference/figures/ess_logo.png)
...
You can also download several rounds by supplying the number of rounds.
...
because if you supply a non existent round, the function will return an
error.
...
[`import_all_rounds()`](https://docs.ropensci.org/essurvey/reference/import_rounds.md).
...
to see available countries and `show_country_rounds("Denmark")` to see
available rounds for chosen country. Alternatively, use
[`import_all_cntrounds()`](https://docs.ropensci.org/essurvey/reference/import_country.md)
to download all available rounds of a country.
...
type [`getwd()`](https://rdrr.io/r/base/getwd.html)). This works the
same way for
[`download_country()`](https://docs.ropensci.org/essurvey/reference/import_country.md).
Be aware that if you download the files manually you should read them
into R with the `haven` package for all `essurvey` related functions to
work.
